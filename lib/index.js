...
let testHexStr = 'ABCDEF0123456789'
/*
let test = per.encSync(testHexStr, 8, {reverse:true, buff: [1,2]});
console.log(test)
console.log(per.decSync(test.data, test.key, test.iterations, {reverse:true, buff: [1,2]}))

per.enc(testHexStr, 8, {reverse:true, buff: [1,2]}, function(err, res){
  if(err){return console.log(err)}
  console.log(res)
  per.dec(res.data, res.key, res.iterations, {reverse:true, buff: [1,2]}, function(err, i){
    if(err){return console.log(err)}
    console.log(i)
  })
})

per.encP(testHexStr, 8, {reverse:true, buff: [1,2]}).then(function(res){
  console.log(res)
  per.decP(res.data, res.key, res.iterations, {reverse:true, buff: [1,2]}).then(function(res){
    console.log(res)
  }).catch(function(err){
    console.log(err)
  })
